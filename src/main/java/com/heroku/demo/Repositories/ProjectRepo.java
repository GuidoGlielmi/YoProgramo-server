...
public interface ProjectRepo extends JpaRepository<Projects, Long> {
  ...
  Object[] getProjects();
  /* SELECT p, techs, urls FROM Projects p INNER JOIN projects_technologies techs ON p.id = techs.project INNER JOIN ProjectUrls urls ON p.id = urls.project ORDER BY title */
}
